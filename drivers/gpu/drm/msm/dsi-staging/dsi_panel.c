	...
	u32 bl_lvl)
{
	...
	if (panel->bl_config.dcs_type_ss || panel->bl_config.dcs_type_r66456)
...
int dsi_panel_enable_doze_backlight(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	if (panel->fod_backlight_flag) {
		...
		if (bl_lvl == 0 && panel->bl_config.dcs_type_r66456)
...
static int dsi_panel_parse_bl_config(struct dsi_panel *panel)
{
	...
	panel->bl_config.dcs_type_r66456 = utils->read_bool(utils->data,
								"qcom,mdss-dsi-bl-dcs-type-r66456");
...
int dsi_panel_set_nolp(struct dsi_panel *panel)
{
	...
	if (panel->bl_config.dcs_type_r66456)
	...
	case DISPPARAM_FOD_BACKLIGHT:
		...
		if (fod_backlight == 0x1000) {
			...
			if (panel->bl_config.local_hbm_flag) {
				pr_info("FOD r66456 local hbm flag off\n");
				rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_DISP_HBM_FOD_OFF);
				break;
			} else {
			...
			if (panel->bl_config.local_hbm_flag) {
				pr_info("FOD r66456 local hbm flag on\n");
